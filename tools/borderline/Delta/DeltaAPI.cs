...
namespace LibSxS.Delta
{
    public static class DeltaAPI
    {
        ...
        public static unsafe byte[] LoadManifest(string path)
        {
        ...
        public static XElement GetManifest(string path)
        {
            var output = LoadManifest(path);

            using (MemoryStream mStr = new MemoryStream(output))
            {
                return borderline.Helpers.StripNamespaces(XElement.Load(mStr));
            }
        }
